;;;;
;;;; This code poorly implements a coloring theme that I like in emacs for CL.
;;;; It is under the MIT license.
;;;; Written by Peter Keller <psilord@cs.wisc.edu>
;;;;
...
(when (require 'color-theme nil 'noerror)
  (if window-system
      (progn
        ;; comment in below line for emacs23, leave commented out for emacs 24
...
(defface ansi-lisp-quoted-symbols
  '((t (:foreground "Tan")))
  "Color all of the quoted symbols")
            ...
            (lambda ()
              (font-lock-add-keywords
               ...
               (append
                ...
                '(("\\([#][BbXxoO][0-9A-Fa-f]+\\)" 0 #'ansi-lisp-numbers))

                ;; Lisp Numbers, simple ones
                '(("\\<\\([+-]?[0-9]+?[.]?[0-9]+\\)\\([ed]\\([+-]?\\)[0-9]+\\)?\\>" 0 #'ansi-lisp-numbers))
                '(("\\<\\([+-]?[.]?[0-9]+\\([ed]\\([+-]?\\)[0-9]+\\)?\\)\\>" 0 #'ansi-lisp-numbers))

                ;; I'm a psycho and want my parentheis color to be controlled.
                '(("\\([()]\\)" 0 #'ansi-lisp-parenthesis))

                ;; Quoted symbols get highlighted:
                '(("'\\([^ :]+:\\(:\\)?\\)?[-a-zA-Z_][-a-zA-Z0-9_]*\\>" 0 #'ansi-lisp-quoted-symbols))
